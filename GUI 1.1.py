# -*- coding: utf-8 -*-
"""
Created on Thu Sep 22 10:10:55 2016
...
def Graph():
    global Lista_Position, Lista_Indice, Lista_Cluster, List_Countries
    global g, h, cluster_centers, Flag_5, Rango, Var_G5, Var_G6
    
    if Flag_5 == 0:
        n_components = 2
        if Var_G5.get() == 0:
            Trans = manifold.TSNE(n_components=n_components, init='pca', random_state=0)
        else:
    ...
    if Var_G5.get() == 0:
        f.canvas.set_window_title('TSNE')
    else:
        f.canvas.set_window_title('MDS')  
    k = 0
...
def Last():
    ...
    for i in range(0,len(Rango),1):                                                 #Iteracion por el numero de filas
        for j in range(0,len(Rango),1):                                             #Iteracion por el numero de Columnas
            ...
            Color_1 = '#%02x%02x%02x' % (Aux_2, Aux_2, Aux_2)                       #Color de la celda proporcional al valor de la distancia, en escala de grises
            Color_2 = '#%02x%02x%02x' % (255-Aux_2, 255-Aux_2, 255-Aux_2)           #Color para el texto inverso al color de la celda
...
B_0 = Tkinter.Button(top, text ="Open File"         ,  command = Look)
B_1 = Tkinter.Button(top, text ="Select Sheet"      ,  command = Show)
B_2 = Tkinter.Button(top, text ="Select Variable"   ,  command = create_window)
B_3 = Tkinter.Button(top, text ="Create Table"      ,  command = tabla)
B_4 = Tkinter.Button(top, text ="Run"               ,  command = Last)
B_5 = Tkinter.Button(top, text ="Create Graph"      ,  command = Graph)

K     = 0
for i in [B_0, B_1, B_2, B_3, B_4, B_5] :
    i.pack()
